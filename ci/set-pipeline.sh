...
fly -t ci set-pipeline -p hello-go-deploy-gce -c pipeline.yml \
    ...
    --var "gce-universal-key-for-all-vms_file=$(cat $HOME/.ssh/gce-universal-key-for-all-vms | base64)" \
    --var "gce-universal-key-for-all-vms_pub_file=$(cat $HOME/.ssh/gce-universal-key-for-all-vms.pub | base64)"
